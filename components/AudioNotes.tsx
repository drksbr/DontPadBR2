...
export function AudioNotes({ documentId, subdocumentId }: AudioNotesProps) {
    ...
    // Get audio notes from Y-Sweet - este hook já é reativo
    const audioNotesArray = useArray(subdocumentId ? `subdocuments.${subdocumentId}.audioNotes` : 'audioNotes')

    // Converter array Y-Sweet para array normal para o render
    const audioNotes = audioNotesArray ? (audioNotesArray.toArray() as AudioNote[]) : []
    ...
    const handleSaveAudio = async (audioBlob: Blob, duration: number) => {
        try {
            ...
            console.log('[AudioNotes] handleSaveAudio - blob size:', audioBlob.size, 'type:', audioBlob.type)
            ...
            console.log('[AudioNotes] Enviando POST para /api/audio-notes')
            const response = await fetch('/api/audio-notes', {
                method: 'POST',
                body: formData,
            })

            console.log('[AudioNotes] Response status:', response.status)

            if (!response.ok) {
                const errorText = await response.text()
                console.error('[AudioNotes] Response error:', errorText)
                throw new Error('Falha ao salvar nota de áudio')
            }

            const newNote = await response.json()
            console.log('[AudioNotes] Nota salva:', newNote)

            // Add to Y-Sweet array
            if (audioNotesArray) {
                audioNotesArray.push([newNote])
                console.log('[AudioNotes] Nota adicionada ao Y-Sweet array')
            } else {
                console.warn('[AudioNotes] audioNotesArray não está disponível')
            }
    ...
    const handleDeleteAudio = async (noteId: string) => {
        try {
            ...
            console.log('[AudioNotes] Deletando áudio:', noteId)
            ...
            console.log('[AudioNotes] Delete response status:', response.status)
            ...
            if (audioNotesArray) {
                ...
                console.log('[AudioNotes] Encontrado índice:', index)
                if (index !== -1) {
                    audioNotesArray.delete(index)
                    console.log('[AudioNotes] Nota removida do Y-Sweet array')
                }
            }
        } catch (err) {
    ...
    const playAudio = async (noteId: string) => {
        try {
            ...
            if (playingId === noteId) {
                console.log('[AudioNotes] Parando áudio:', noteId)
            ...
            if (playingId !== null && currentAudioRef.current) {
                console.log('[AudioNotes] Parando áudio anterior:', playingId)
                currentAudioRef.current.pause()
                currentAudioRef.current.currentTime = 0
            }

            setPlayingId(noteId)
            console.log('[AudioNotes] Playing audio:', noteId, 'documentId:', documentId, 'subdocumentId:', subdocumentId)
            ...
            console.log('[AudioNotes] Play response status:', response.status)

            if (!response.ok) {
                const errorText = await response.text()
                console.error('[AudioNotes] Play response error:', errorText)
                throw new Error('Falha ao carregar áudio')
            }

            const audioBlob = await response.blob()
            console.log('[AudioNotes] Audio blob received, size:', audioBlob.size)
            ...
            audio.onended = () => {
                console.log('[AudioNotes] Áudio terminado:', noteId)
                setPlayingId(null)
                    ...
                    audioNotes.map(note => (
                        ...
                        >
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="flex-shrink-0 w-8 h-8 rounded text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition opacity-0 group-hover:opacity-100 disabled:cursor-not-allowed flex items-center justify-center"
                                title="Deletar"
                                aria-label="Deletar"
                            >
                                <svg
